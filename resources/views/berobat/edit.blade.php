    ...
    <div class="mx-2 mt-3 pt-4 bg-light shadow rounded">
        <div class="row">
            <div class="col-sm-6">
                ...
                <p class="ps-3 text-muted">Edit data pasien berobat</p>
            </div>
        </div>
        <hr>
        ...
        <div class="container">
            <div class="row my-4">
                <div class="col-md-5">
                    <form action="{{ '/berobat/'.$data->id }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('nama_pasien_id') is-invalid @enderror" name="nama_pasien_id" id="nama_pasien_id">
                                ...
                                @foreach ($pasien as $item)
                                    <option value="{{ $item->id }}" {{ $data->nama_pasien_id == $item->id ? 'selected' : ''}}>{{ $item->nama_pasien }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control @error('tanggal_berobat') is-invalid @enderror" id="tanggal_berobat" name="tanggal_berobat" value="{{ $data->tanggal_berobat }}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="keluhan" id="keluhan" class="form-control @error('keluhan') is-invalid @enderror">{{ $data->keluhan }}</textarea>
                            @error('keluhan')
                                <div class="invalid-feedback text-start">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('nama_dokter_id') is-invalid @enderror" name="nama_dokter_id" id="nama_dokter_id">
                                ...
                                @foreach ($dokter as $item)
                                    <option value="{{ $item->id }}" {{ $data->nama_dokter_id == $item->id ? 'selected' : ''}}>{{ $item->nama_dokter }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('hasil_periksa') is-invalid @enderror" id="hasil_periksa" name="hasil_periksa" value="{{ $data->hasil_periksa }}">
                            @error('hasil_periksa')
                                <div class="invalid-feedback text-start">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check form-check-inline">
                                <input class="form-check-input @error('confirm_rujuk') is-invalid @enderror" type="radio" name="confirm_rujuk" id="1" value="1" {{ $data->confirm_rujuk == '1' ?  'checked' : ''}} onChange="checkOption(this)">
                                <label class="form-check-label" for="1">Ya</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input @error('confirm_rujuk') is-invalid @enderror" type="radio" name="confirm_rujuk" id="0" value="0" {{ $data->confirm_rujuk == '0' ?  'checked' : ''}} onChange="checkOption(this)">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('nama_obat_id') is-invalid @enderror" name="nama_obat_id" id="nama_obat_id">
                                ...
                                @foreach ($obat as $item)
                                    <option value="{{ $item->id }}" {{ $data->nama_obat_id == $item->id ? 'selected' : ''}}>{{ $item->nama_obat }}</option>
                                @endforeach
                            </select>
                            @error('nama_obat_id')
                                <div class="invalid-feedback text-start">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('nama_rs_id') is-invalid @enderror" name="nama_rs_id" id="nama_rs_id">
                                ...
                                @foreach ($rs_rujuk as $item)
                                    <option value="{{ $item->id }}" {{ $data->nama_rs_id == $item->id ? 'selected' : ''}}>{{ $item->nama_rs }}</option>
                                @endforeach
                            </select>
                            @error('nama_rs_id')
                                <div class="invalid-feedback text-start">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('biaya') is-invalid @enderror" id="biaya" name="biaya" value="{{ $data->biaya }}">
                            @error('biaya')
                                <div class="invalid-feedback text-start">
                                    {{ $message }}
                                </div>
